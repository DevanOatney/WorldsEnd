...
public class ItemLoaderScript : MonoBehaviour 
{
	...
	void LoadArmorItems()
	{
		...
		foreach(string line in lines)
		{
			...
			armor.m_nModifier = int.Parse(szPieces[2].Trim());
			armor.m_nBaseValue = int.Parse(szPieces[3].Trim());
			armor.m_nSpecialType = int.Parse(szPieces[4].Trim());
			armor.m_nSpecialModifier = int.Parse(szPieces[5].Trim());
			armor.m_szDescription = szPieces[6].Trim();
